...
- **INFO** `math_inline_normalized`: Convertito `\(...\)` in `$...$`.
- Fix suggerito: Mantieni una sola convenzione per la matematica inline.
...
- **INFO** `table_normalized`: Tabella pipe normalizzata per Pandoc.
- Estratto: `| Property | Small / Simple $\mathcal{H}$ | Large / Complex $\mathcal{H}$ |`

- **INFO** `table_separator_inserted`: Inserita riga separatrice standard in una pipe table.
- Estratto: `| Hypothesis Class | Domain | VC-Dimension | Explanation |`

- **INFO** `table_normalized`: Tabella pipe normalizzata per Pandoc.
- Estratto: `| Feature | Ridge Regression (L2) | Lasso Regression (L1) |`
...
- **INFO** `table_normalized`: Tabella pipe normalizzata per Pandoc.
- Estratto: `| Feature               | L2 Regularization (Ridge)                     | L1 Regularization (Lasso)                          |`

- **INFO** `table_normalized`: Tabella pipe normalizzata per Pandoc.
- Estratto: `| Feature | **Boosting** (e.g., AdaBoost, XGBoost) | **Bagging** (e.g., Random Forest) |`

- **INFO** `table_normalized`: Tabella pipe normalizzata per Pandoc.
- Estratto: `| **Pros** | **Cons** |`

- **INFO** `table_normalized`: Tabella pipe normalizzata per Pandoc.
- Estratto: `| **Variant** | **Data used per step** | **Gradient Accuracy** | **Speed per step** | **Convergence** |`

- **INFO** `table_normalized`: Tabella pipe normalizzata per Pandoc.
- Estratto: `| **Feature** | **Batch GD** | **Stochastic GD (SGD)** | **Mini-Batch GD** |`

- **INFO** `table_normalized`: Tabella pipe normalizzata per Pandoc.
- Estratto: `| **Feature** | **KNN (Supervised)** | **K-Means (Unsupervised)** |`

- **INFO** `table_normalized`: Tabella pipe normalizzata per Pandoc.
- Estratto: `| **Feature** | **K-Means** | **Hierarchical (Agglomerative)** |`

- **INFO** `table_normalized`: Tabella pipe normalizzata per Pandoc.
- Estratto: `| **Feature** | **FNN (Dense)** | **CNN (Convolutional)** |`

- **INFO** `table_normalized`: Tabella pipe normalizzata per Pandoc.
- Estratto: `| **Feature** | **PCA** | **Autoencoder (Deep)** |`
...
```bash
pandoc --from markdown+yaml_metadata_block+wikilinks_title_after_pipe+tex_math_dollars+pipe_tables+grid_tables+multiline_tables+raw_tex+smart --resource-path /tmp/pandoc-guard-bO6DqN:/home/nicola/Documents/Local Obs/Obsidian Locale -s /tmp/pandoc-guard-bO6DqN/Appunti Nicola.sanitized.md -t latex -o /tmp/pandoc-guard-bO6DqN/Appunti Nicola.sanitized.tex
```

## Comando motore PDF

```bash
xelatex -interaction=nonstopmode -halt-on-error -output-directory /tmp/pandoc-guard-bO6DqN /tmp/pandoc-guard-bO6DqN/Appunti Nicola.sanitized.tex
```
